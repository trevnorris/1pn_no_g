...
- The code follows that structure: incompressible forces implement \(\mathbf F=\rho_0 Q\,\mathbf v_{\rm ext}\), while `slab/surface.py` adds the compressible correction derived from Eq. (7) of `1pn_no_g.tex` by evaluating the renormalised surface integral with the Rule A/Rule B bookkeeping. The analytic helper still delegates to the quadrature path each timestep, so production runs pay the full surface-integration cost.【F:slab/surface.py†L460-L705】【F:1pn_no_g.tex†L260-L309】
- Unit tests confirm the compressible correction obeys the expected scalings (vanishes as \(c_s\to\infty\), follows \(c_s^{-2}\), remains Galilean-invariant) but we have not yet demonstrated a quantitative match to the EIH 1PN excess along a Mercury trajectory—closing that gap requires longer, better-resolved integrations rather than swapping in the GR formula directly.【F:test_compressible_forces.py†L1-L212】
